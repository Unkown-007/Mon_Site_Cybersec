...
export const tier4Tests: Test[] = [
  {
    ...
    run: async (): Promise<TestResult> => {
      const exists = await checkFileExists('next.config.mjs');
      if (exists) {
        const content = await readSourceFile('next.config.mjs');
  ...
  },
  {
    ...
    type: 'static',
    run: async (): Promise<TestResult> => {
      const loginContent = await readSourceFile('src/app/login/page.tsx');
  ...
  },
  {
    ...
    run: async (): Promise<TestResult> => {
      try {
        ...
        return { id: 'T4-S-3', passed: false, message: 'Runtime check failed', details: (err as Error).message };
      }
    },
  },
  {
    ...
    run: async (): Promise<TestResult> => {
      try {
        ...
        return { id: 'T4-S-4', passed: false, message: 'Runtime check failed', details: (err as Error).message };
      }
    },
  },
  {
    ...
    run: async (): Promise<TestResult> => {
      try {
        ...
        return { id: 'T4-S-5', passed: false, message: 'Runtime check failed', details: (err as Error).message };
      }
    },
  },
];
